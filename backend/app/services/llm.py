...
async def llm_canonicalize(cv_text: str, extraction_warnings: list[str]) -> CVCanonical:
    """
    ...
    if not client:
        return CVCanonical(
            summary=None,
            extraction_warnings=extraction_warnings + ["OpenAI API key non configurée."],
        )
...
Réponds UNIQUEMENT avec un JSON valide, sans texte avant ou après."""
    ...
    try:
        ...
        if "429" in error_msg or "insufficient_quota" in error_msg.lower():
            ...
            user_message = "Clé API OpenAI invalide. Vérifiez votre configuration dans le fichier .env"
...
async def llm_ats_and_insights(canonical: CVCanonical) -> tuple[ATSAssessment, CVInsights]:
    """
    ...
    if not client:
        ats = ATSAssessment(
            ...
            issues=["OpenAI API key non configurée."],
...
Réponds UNIQUEMENT avec un JSON valide avec les clés "ats" et "insights"."""
    ...
    try:
        ...
        ats = ATSAssessment(
            ...
            issues=[user_message],
            quick_wins=["Rechargez votre compte OpenAI pour obtenir une analyse complète"],
        )
        insights = CVInsights(
            positioning=[],
            strengths=[],
...
async def llm_job_matching(canonical: CVCanonical, job_description: str) -> JobMatching:
    """
    ...
    if not client:
        return JobMatching(
            ...
            experience_match=0,
            education_match=0,
            missing_requirements=["OpenAI API key non configurée."],
            strengths=[],
            recommendations=[],
        )
...
Réponds UNIQUEMENT avec un JSON valide."""
    ...
    try:
        ...
        return JobMatching(
            ...
            strengths=[],
            recommendations=["Rechargez votre compte OpenAI sur https://platform.openai.com/account/billing"],
        )

async def llm_keyword_matching(canonical: CVCanonical, job_description: str) -> KeywordMatching:
    """
    ...
    if not client:
        return KeywordMatching(
            keywords=[],
            coverage_score=0,
            critical_missing=["OpenAI API key non configurée."],
        )
...
Réponds UNIQUEMENT avec un JSON valide."""
    ...
    try:
        ...
        # Construire la liste des KeywordMatch
        keywords_data = result.get("keywords", [])
        keyword_matches = []
        
        for kw_data in keywords_data:
            try:
                status_str = kw_data.get("status", "absent").lower()
                ...
                category_str = kw_data.get("category", "other").lower()
                category_map = {
                ...
                keyword_matches.append(KeywordMatch(
                    ...
                    importance=min(5, max(1, int(kw_data.get("importance", 3)))),
                ))
            except Exception:
        ...
        return KeywordMatching(
            ...
            coverage_score=min(100, max(0, int(result.get("coverage_score", 0)))),
            critical_missing=result.get("critical_missing", []) if isinstance(result.get("critical_missing"), list) else [],
        )
    except json.JSONDecodeError as e:
        return KeywordMatching(
            keywords=[],
...
async def llm_compare_cvs(cvs_data: list[tuple[str, str, CVCanonical]], job_description: str) -> CVComparisonResult:
    """
    ...
    if not client:
        return CVComparisonResult(
            ...
            overall_ranking=[],
            criteria_comparison=[],
            summary="OpenAI API key non configurée.",
        )
    ...
    for cv_id, filename, canonical in cvs_data:
        cvs_info.append({
            "id": cv_id,
            "filename": filename,
            "data": canonical.model_dump_json()[:4000]  # Limiter la taille
        })
    
    # Construire une description claire des CVs avec leurs IDs
    cvs_description = []
    for i, cv_info in enumerate(cvs_info):
        cvs_description.append(f"{i+1}. CV ID: '{cv_info['id']}' - Fichier: {cv_info['filename']}")
    
    # Liste des IDs pour référence
...
CRITIQUE: Utilise EXACTEMENT les IDs fournis (ex: "{cvs_data[0][0]}" pour le premier CV). Réponds UNIQUEMENT avec un JSON valide."""
    ...
    try:
        ...
        # Construire les CVComparisonItem
        cvs_items = []
        cvs_dict = {cv_id: (filename, canonical) for cv_id, filename, canonical in cvs_data}
        ...
        # Extraire les données des CV depuis le résultat
        cvs_result = result.get("cvs", [])
        
        # Si le format est différent, essayer de récupérer par ID
        for cv_id, filename, canonical in cvs_data:
            # Chercher les données correspondantes dans le résultat
            cv_result_data = None
            for cv_res in cvs_result:
                # Le LLM peut retourner l'ID ou le filename comme identifiant
                if (cv_res.get("cv_id") == cv_id or 
                    cv_res.get("filename") == filename or
                    cv_res.get("id") == cv_id):
                    cv_result_data = cv_res
                    break
            
            # Si pas trouvé, utiliser les données par défaut ou la première entrée
            if not cv_result_data and cvs_result:
                # Essayer de mapper par position
                idx = cvs_data.index((cv_id, filename, canonical))
                if idx < len(cvs_result):
                    cv_result_data = cvs_result[idx]
            
            if cv_result_data:
                cvs_items.append(CVComparisonItem(
                    cv_id=cv_id,
                    filename=filename,
                    canonical=canonical,
                    ...
                    justification=cv_result_data.get("justification", ""),
                    keyword_matching=None,  # Sera rempli par l'API
                ))
            else:
                # Fallback : créer un item avec scores par défaut
                cvs_items.append(CVComparisonItem(
                    cv_id=cv_id,
                    filename=filename,
                    canonical=canonical,
        ...
        # Construire les ComparisonCriterion
        criteria = []
        for crit_data in result.get("criteria_comparison", []):
            criteria.append(ComparisonCriterion(
                criterion_name=crit_data.get("criterion_name", ""),
                best_cv_id=crit_data.get("best_cv_id", ""),
                ranking=crit_data.get("ranking", []) if isinstance(crit_data.get("ranking"), list) else [],
                justification=crit_data.get("justification", ""),
            ))
        
        # Construire le classement global si manquant
        overall_ranking = result.get("overall_ranking", [])
        if not overall_ranking or not isinstance(overall_ranking, list):
            # Créer un classement basé sur les scores
